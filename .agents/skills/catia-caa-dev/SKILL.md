...
本文档很长。**不要默认通读全文**——先读必读区，再按当前任务从下表定位单个章节，只读那一节。

**必读区（每次都要读）：**

1. Frontmatter（触发词与路由）
2. ⛔ 强制工作流（上方引用块）
3. 本节（阅读地图）

**按任务定位章节（章节标题是稳定锚点，行号会漂移，不要依赖行号）：**

| 任务 | 阅读章节 |
|------|----------|
| 看能创建/生成什么（命令、对话框、工作台等） | `## 📚 支持的模板类型` |
| CLI / MCP / Python API 三者怎么配合 | `## 🔌 三接口协作` |
| Python API 具体调用、参数、返回值 | `## 🔧 Python API` |
| 编译、构建、Build Time 命令、启动 CATIA | `## 🛠️ 命令行工具` |
| 运行测试、看测试体系 | `## 📊 测试验证` |
| 查某个文件/目录是干什么的 | `## 📁 文件结构` |
| 文档该放哪、命名规范 | `## 📁 项目文档规范` |
| 常见错误、正确用法对比 | `## 🎓 最佳实践` |
| 报错、异常、构建失败的排查路径 | `## 🔍 故障排查`（再配合 `docs/FAQ.md`） |
| 当前能力边界、未就绪项 | `## 🚧 生产就绪评估` |
| 版本历史、变更记录 | `CHANGELOG.md` |

**规则：**

1. 必读区始终先读；专项章节只在任务需要时读对应那一节。
2. 拿不准读哪节 → 先读 `## 🎯 核心特性` 开头再决定。
3. 章节标题找不到或内容对不上 → 用 grep 按章节标题或关键词定位，不要顺序通读。
4. 本文件同时是仓库结构的注册表（版本号、文件树、能力名被测试对账），**不要移动或重命名章节**；新增内容追加到对应章节内部。
...
| 🧭 **核实"类型有没有某方法"用 method_index** | 不确定某接口是否真有某方法（尤其"看起来合理"的遍历/Get 方法，如  就是虚构）时，跑 `python skills/method_index.py <Type> <Method1> [Method2 ...]`：OK=方法真实（含继承链），NOT-FOUND 会同时给出**该类型上的相近方法名**和**真正拥有该方法的接口列表**（如 GetAllChildren → CATIProduct/CATIParmPublisher）。比翻 CAADoc 页面快一个数量级。 |
...
- **Never infer capability from file existence** — 路由查 `capabilities.yaml`（意图→能力→binding），存在性查 `skills/lifecycle.yaml`（防 Phantom）
