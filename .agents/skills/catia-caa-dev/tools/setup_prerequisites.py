#!/usr/bin/env python3
"""
CADE Workspace Prerequisites Manager
=====================================
Automatically configure workspace prerequisites (equivalent to CATIA's "Manage prerequisites" dialog).
    ...
    if sys.platform == "win32":
        try:
            ...
            key = winreg.OpenKey(
                ...
                r"SOFTWARE\Dassault Systemes\CATIA"
            )
            i = 0
            while True:
                try:
...
def setup_workspace_prerequisites(
    workspace: Path,
    ...
    """
    Configure workspace prerequisites.
    ...
    if not validate_catia_root(catia_root):
        return {
            "status": "error",
            "message": f"Invalid CATIA root: {catia_root}"
        }
    ...
    if cnext_exe is None:
        return {
            "status": "error",
            "message": f"CNEXT.exe not found in {catia_root}"
        }
    ...
    try:
        ...
        return {
            "status": "ok",
            "message": f"Workspace prerequisites configured successfully",
            "config": config
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CADE Workspace Prerequisites Manager"
    )
    parser.add_argument(
        "workspace",
        nargs="?",
    ...
    parser.add_argument(
        "--catia-root",
        help="CATIA installation path (auto-detected if not specified)"
    )
    parser.add_argument(
        "--copy-prereq",
        action="store_true",
        help="Copy prerequisite files locally"
    )
    parser.add_argument(
        "--target-dir",
        help="Target directory for copied prerequisites"
    )
    parser.add_argument(
        "--show",
        action="store_true",
