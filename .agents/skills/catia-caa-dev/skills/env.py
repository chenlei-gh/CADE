...
class CAAEnvironment:
    ...
    def __init__(self, config_file: Optional[str] = None):
        ...
        self._rules = None
        self._tck_id = None
        # Path to the temp .bat materialized by the most recent
        # build_time_command() call. Owned by the caller — see
        # cleanup_build_bat().
        self._build_bat: Optional[Path] = None
    ...
    def build_time_command(
        ...
        Ownership:
            This call has a side effect: it materializes a temp .bat under
            %TEMP% (prefix "cade_build_") and the returned command is
            ["cmd", "/c", <that .bat>]. cmd[2] is therefore the absolute path
            to the generated file.

            The CALLER owns that file and must remove it once the command has
            been executed (or once it is known it never will be), by calling
            cleanup_build_bat(). Nothing else deletes it: the .bat is not
            self-deleting, because in the command-generation-only usage the
            file is never executed at all.

            self._build_bat always points at the most recently generated
            file, so cleanup must happen after EACH call — a later call
            overwrites the slot, and an earlier file that was never cleaned
            becomes an orphan whose only remaining handle is cmd[2].
        """
        catia_path = Path(self.config.get("CATIA_INSTALL", ""))
        arch = self._detect_architecture(catia_path) or "win_b64"
        code_bin = catia_path / arch / "code" / "bin"
    ...
    def cleanup_build_bat(self) -> bool:
        """Remove the temp .bat generated by the last build_time_command().

        Idempotent and safe to call unconditionally: it is a no-op when no
        file was generated, when it has already been removed (by this method
        or by hand), or when a previous call already succeeded.

        On failure the path is retained in self._build_bat so a later retry
        can still succeed; the failure is reported on stderr rather than
        swallowed, otherwise a recurring cleanup failure would be invisible
        in logs.

        Returns:
            True if the file was removed (or nothing needed removing),
            False if removal was attempted and failed.
        """
        if self._build_bat is None:
            return True
        try:
            if self._build_bat.exists():
                self._build_bat.unlink()
        except OSError as e:
            print(
                f"WARNING: Failed to remove temp build .bat "
                f"{self._build_bat}: {e}",
                file=sys.stderr,
            )
            return False
        self._build_bat = None
        return True
...
def get_build_command(workspace_path: str, options: str = "-u -a") -> Tuple[list, str]:
    ...
    .. deprecated::
        This helper cannot uphold the ownership contract of
        build_time_command(). That method materializes a temp .bat under
        %TEMP% and expects the CALLER to remove it via cleanup_build_bat(),
        but the local CAAEnvironment instance created here is discarded the
        moment this function returns. Ownership can therefore never reach
        the caller, and the generated .bat is always orphaned.

        The lifecycle cannot be repaired with __del__ or weakref.finalize
        either: the file has to stay alive for the returned command to be
        executable, so those hooks would delete a file that is still in use.

        Use CAAEnvironment.build_time_command() directly and call
        cleanup_build_bat() when done. No in-repo caller uses this helper.
    """
    caa_env = CAAEnvironment()
