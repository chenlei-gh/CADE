...
def build_workspace(
    ...
    effective_target_module = target_module or (workspace_path.name if workspace_path.name.endswith(".m") else None)
    ...
    try:
        ...
        res = error_result(
            f"Build timeout after {timeout} seconds",
            timeout=timeout,
            entrypoint=entrypoint,
            orchestrated_by_kernel=orchestrated_by_kernel,
            operation="build",
...
def main():
    ...
    parser.add_argument(
        "--module", "-m", default=None, help="Target module name (e.g. CAABOMToolCmd.m)"
    )
    parser.add_argument(
        "--skip-gate",
        action="store_true",
