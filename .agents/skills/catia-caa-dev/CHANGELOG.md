...
- **回归验证 (DC1～DC24 全量覆盖)**：
  ...
  - **DC17～DC20 深度加固回归**：
    - **DC17**：外部 ChangeSet 已有目标 Addin 的 `modified` 暂存时阻断（返回 `CHANGESET_CONFLICT`）；
...
### 已修复
- **P0 安全修复（6/6）**：ChangeSet 路径验证、原子 apply、删除文件回滚、Backup 路径遍历防护、Repair 直接写入防护
- **P1 核心功能修复（8/8）**：MacDeclareHeader 模板模式、Patch 序列化往返、IDL 接口初始化、构建验证失败状态、目标模块 DLL 检查、Patch 匹配断言、ChangeSet 合并冲突检测
- **P2 基础设施修复（9/9）**：分析器（接口/工作台检测）、VS 检测（vswhere）、工作区隔离缓存、mktemp 安全、命令注入加固、前置条件失败传播、多 Framework 同步
- **P3 测试质量修复（10/10）**：诚实指标（移除 100% 覆盖率声明）、退出码验证、隔离断言、临时工作区清理、版本同步

### 已变更
- SKILL.md：准确套件数量（38 套件）、状态更新为"活跃开发"、移除"无已知 Bug"声明
- 版本：所有文件统一为 **3.2.1**（cade.py、mcp_server.py、SKILL.md、CHANGELOG.md、production_readiness_check.py）
