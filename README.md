...
传统 CAA 开发门槛高、RADE 向导繁琐、头文件与宏规则复杂、编译报错难以定位。CADE 将自然语言需求转化为标准的 CAA 工程资产，串联起 **需求澄清 → 规划生成 → 静态验证 → mkmk 构建 → CATIA 运行 → 诊断修复 → 变更回滚** 的完整开发闭环。
...
| **反复查阅**：官方 SDK / CAADoc 查阅低效 | **知识检索**：结构化知识体系与真实 API 校验防幻觉 |
| **人工排错**：编译失败后手动查日志、猜原因 | **闭环修复**：构建 → 验证 → 修复全自动管线 |
| **重构高危**：重命名极易漏改 `.dico`、NLS 与 Imakefile | **安全重构**：ChangeSet 变更预览、备份与可逆回滚 |
...
- `repair()`：编译报错分类、修复计划执行、组件重构、回滚恢复。
...
git clone https://github.com/chenlei-gh/CADE.git
Copy-Item -Recurse .\CADE\.agents C:\你的CAA工作区\
pip install Pillow
```

- **Zed**：开箱即用（自动读取 `.agents/skills/`）。
...
CADE 自动完成：
1. **生成组件结构**：`MyAnalysisCmd.h`、`MyAnalysisCmd.cpp`、`Imakefile.mk`、`.dico` 字典、NLS 资源与图标。
2. **执行闭环管线**：澄清意图 → 规划任务 → 生成脚手架 → 静态验证 → mkmk 构建 → 启动 CNEXT 验证。
